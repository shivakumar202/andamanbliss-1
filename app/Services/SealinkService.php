...
class SealinkService
{
    ...
    {
        $this->baseUrl = env('SEALINK_API_BASE_URL');
        $this->username = env('SEALINK_API_USERNAME');
        $this->token;
    }
    ...
    {
        $this->token = config('services.sealink.token');
        return env('SEALINK_API_TOKEN');
    }
    /**
    ...
    public function login()
    {
        $response = Http::post("{$this->baseUrl}login", [
            'data' => [
                ...
                'token' => 'U2FsdGVkX1/TTiNjBr56FyStW7j5VzaWIhPXZa2LCWwaqmCyAPh1hsy/z38dg4GQqzJdZ8Fs7jemO8csHUhZFw/QOMMG2ZEgjqIQhg8bsOc=',
            ],
        ]);
    ...
    public function getTripData($data)
    {
        try {
            ...
            $url = rtrim(env('SEALINK_API_BASE_URL'), '/') . '/getTripData';

            Log::info('Sending Sealink Trip Data Request:', ['url' => $url, 'payload' => $formattedData]);

            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . env('SEALINK_API_TOKEN'),
    ...
    public function bookSeats($bookingData)
    {
        try {
            ...
            $url = rtrim(env('SEALINK_API_BASE_URL'), '/') . '/bookSeats';
    ...
    public function bookSeatsAuto(array $autoBookingData)
    {
        try {
            ...
            $url = rtrim(env('SEALINK_API_BASE_URL'), '/') . '/bookSeatsAuto';
    ...
    public function getBalance()
    {
        try {
            ...
            $url = rtrim(env('SEALINK_API_BASE_URL'), '/') . '/getProfile';
