    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            <form name="form" action="{{ url('admin/cabs') }}" method="GET" id="form" class=""
              ...
              <div class="row align-items-center justify-content-center">
                ...
                <div class="align-self-center col-md-2">
                  <a href="{{ url('admin/cabs') }}" class="btn btn-block btn-outline-info my-1">
            ...
            <table id="dataTable" class="table table-striped table-hover w-100">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ @$cab->id }}</td>
                  <td>{{ ucwords(@$cab->name) }}</td>
                  <td>{{ @$cab->category }}</td>                  
                  <td>
                    @foreach ($cab->service_locations as $location)
                    {{$location .','}}
                  @endforeach
                  </td>
                  <td>{{ @$cab->sitting_capacity }} Seater</td>
                  <td>{{ @$cab->fuel_type }}</td>
                  <td>{{ @$cab->luggage }}</td>
                  <td>{{ @$cab->status == '1' ? 'Active' : 'Inactive'}}</td>
                  <td>
                    <a href="{{ url('admin/cabs/' . @$cab->id .'/images') }}" title="Details"
                      class="btn btn-xs btn-dark py-0 mx-1">
                      <i class="fa fa-eye"></i>
                    </a>
                    <a href="{{ url('admin/cabs/' . @$cab->id . '/edit') }}" title="Edit"
                    ...
                    <form action="{{ url('admin/cabs/' . $cab->id) }}" method="POST" class="d-none">
                      @method('DELETE')
                      @csrf
                    </form>
                  </td>
