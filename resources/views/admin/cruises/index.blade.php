    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            <form name="form" action="{{ url('admin/cruises') }}" method="GET" id="form" class=""
              ...
              <div class="row align-items-center justify-content-center">
                ...
                <div class="align-self-center col-md-2">
                  <a href="{{ url('admin/cruises') }}" class="btn btn-block btn-outline-info my-1">
            ...
            <table id="dataTable" class="table table-striped table-hover w-100">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ @$cruise->id }}</td>
                  <td>{{ ucwords(@$cruise->name) }}</td>
                  <td>{{ @$cruise->category->name }}</td>
                  <td>{{ @$cruise->address }}</td>
                  <td>{{ @$cruise->featured == 1 ? 'Yes' : 'No' }}</td>
                  <td>{{ @$cruise->best_seller == 1 ? 'Yes' : 'No' }}</td>
                  <td>{{ @$cruise->ratings }} Star ({{ $cruise->reviews_count }})</td>
                  <td>{{ @$cruise->status == 'active' ? 'Active' : 'Inactive'}}</td>
                  <td>
                    <a href="{{ url('admin/cruises/' . @$cruise->id) }}" title="Details"
                      class="btn btn-xs btn-dark py-0 mx-1">
                      <i class="fa fa-eye"></i>
                    </a>
                    <a href="{{ url('admin/cruises/' . @$cruise->id . '/edit') }}" title="Edit"
                    ...
                    <form action="{{ url('admin/cruises/' . $cruise->id) }}" method="POST" class="d-none">
                      @method('DELETE')
                      @csrf
                    </form>
                  </td>
