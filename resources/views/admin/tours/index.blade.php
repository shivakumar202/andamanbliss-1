    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            <form name="form" action="{{ url('admin/tours') }}" method="GET" id="form" class=""
              ...
              <div class="row align-items-center justify-content-center">
                ...
                <div class="align-self-center col-md-2">
                  <a href="{{ url('admin/tours') }}" class="btn btn-block btn-outline-danger my-1">
            ...
            <table id="dataTable" class="table table-striped table-hover w-100">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ @$tour->id }}</td>
                  <td>{{ ucwords(@$tour->package_name) }}</td>
                  <td>{{ ucwords(@$tour->tourCategory->name) . ' - ' . $tour->tourCategory->rating }} Star</td>
                  <td>{{ @$tour->nights .' nights ' . ' - ' . @$tour->days . ' days' }}</td>
                  <td>{{ implode(', ', $tour->islands_covered ?? []) }}</td>
                  <td>{{ @$tour->featured == 1 ? 'Yes' : 'No' }}</td>                                
                  <td>{{ @$tour->best_seller == 1 ? 'Yes' : 'No' }}</td>
                  <td> ({{ $tour->reviews }})</td>
                  <td>{{ @$tour->status == '1' ? 'Active' : 'Inactive'}}</td>
                  <td>
                    <a href="{{ url('admin/tours/' . @$tour->id) }}" title="Details"
                      class="btn btn-xs btn-dark py-0 mx-1">
                      <i class="fa fa-eye"></i>
                    </a>
                    <a href="{{ url('admin/tours/' . @$tour->id . '/edit') }}" title="Edit"
                    ...
                    <form action="{{ url('admin/tours/' . $tour->id) }}" method="POST" class="d-none">
                      @method('DELETE')
                      @csrf
                    </form>
                  </td>
