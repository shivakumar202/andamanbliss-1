<div class="border p-3 bg-light  shadow priceSummary">
    <form action="#" wire:submit.prevent="submit" enctype="multipart/form-data">
        ...
        <div class="d-flex justify-content-between price-innrer-title">
            <div><span>Bike</span></div>
            <div><span class="fw-semibold fs-6">{{ __(@$bike->name) }}</span></div>
        </div>
        <div class="mt-3 view_page_price CarpriceDetails">
            <div class="row d-flex align-items-center">
                <div class="col-md-6">
                    <h2>Rs.{{ number_format(@$totalCost, 2) }}<span class="person">/Day</span></h2>
                </div>
                <div class="col-md-6">
                    <p class="price-note"><span class="text-danger">Note:</span><span><img
                                src="{{ asset('site/img/insurance.png') }}" class="img-fluid security-depo-img"
                                alt="support" loading="lazy"></span> Refundable Security Deposite <span
                            class="security-amt fw-bold">Rs2000</span></p>
                </div>
            </div>
        </div>
        <div class="mt-3 view_page_price CarpriceDetails">
            <div class="room-collection_details d-flex justify-content-between">
                <span>Location</span>
                <span class="fs-6 fw-semibold">{{ $location }}</span>
            </div>


            <div class="d-flex justify-content-between my-3">
                <div class="room-collection_details">
                    <span>No of Bike(s)</span>
                </div>
                <div>
                    <span>
                        <div class="pCountall adult">
                            <div class="quantity">
                                <a href="#" class="quantity__minus"
                                    wire:click="decrement('bike')"><span>-</span></a>
                                <input type="text" name="quantity" wire:model="bikes" id="adultCount"
                                    class="quantity__input" aria-label="Quantity" readonly>
                                <a href="#" class="quantity__plus"
                                    wire:click="increment('bike')"><span>+</span></a>
                            </div>
                        </div>
                    </span>
                </div>
            ...
            @if (!empty($bike->addon_names))
                ...
                @else 
                    wire:click="toggleAddon({{ $addon->id }})" @endif>
                            ...
                            @endif
                            <p class="fw-bold pt-2">₹{{ $addon->price }} </p>
